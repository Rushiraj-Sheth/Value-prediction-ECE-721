   ...
   if (Execution_Lanes[lane_number].wb.valid) {
      ...
      if (PAY.buf[index].checkpoint) {
         ...
         if (PERFECT_BRANCH_PRED) {
            ...
            ///////////////////////// recover VPQ
            VP->VPQ_recovery(PAY.buf[index].checkpoint_tail, PAY.buf[index].checkpoint_tp);
            //////////////////////// recover VPQ
      ...
      if( !PERF_VP && PAY.buf[index].vp_eligible ){
         ...
         // if( VP->VPQ[ PAY.buf[index].vpq_entry ].PC == 86040 ){
         //    cout<<" writeBACK: "<<"pc_index\t"<<VP->VPQ[ PAY.buf[index].vpq_entry ].PC_index<<"\tvalue\t"<<PAY.buf[index].C_value.dw<<endl;
         // }
      }
