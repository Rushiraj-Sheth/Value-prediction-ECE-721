...
void payload::predict(pipeline_t *proc, uint64_t pc, uint64_t max_length, uint64_t &cb_predictions, uint64_t &indirect_target) {
   ...
   if (tail == head) {
      ...
      // Note that a "bad" instruction can still be observed in the perfect branch prediction mode.
      // One such example is fetching under the shadow of an exception-triggering instruction.
   ...
   // Initialize indirect_target to 0.
   indirect_target = 0;
