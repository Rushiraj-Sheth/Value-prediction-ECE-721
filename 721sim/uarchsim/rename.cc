...
void pipeline_t::rename2() {
   ...
   bundle_dst = 0;
   bundle_branch = 0;
   // there aren't enough VPQ entries
   VP_valid_inst = 0;
   ...
   //Stall Logic for VP
   if( VP->stall_VPQ(VP_valid_inst) && SVP_VP )
   {
      return;
   }
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      ///////////// set all VP PAYLOAD flags to default value ///////////////////
      PAY.buf[index].is_Confident      = false;
      PAY.buf[index].vp_eligible       = false;
      PAY.buf[index].pred_DST.dw       = 0;
      PAY.buf[index].is_Hit            = false;
      PAY.buf[index].is_pred_Correct   = false;
      PAY.buf[index].vpq_entry = 1000;

      
      //////////////// checking for VP eligible /////////////////////////
      bool vp_eligible = eligible( &(PAY.buf[index]) );

      // IF PERFECT VP:
      if( vp_eligible && PERF_VP ){
         PAY.buf[index].vp_eligible = true;
         // check if correct path instr
         // link it to its counterpart in checker simulator
         if( PAY.buf[index].good_instruction ){
            // predict the value of dest reg. PARALLEY
            //PAY.buf[index].good_pred_DST = true;
            
            PAY.buf[index].is_Confident = true;
            PAY.buf[index].is_Hit       = true;

            db_t* db_entry = get_pipe()->peek ( PAY.buf[index].db_index );
            PAY.buf[index].pred_DST.dw = db_entry->a_rdst[0].value;
         }
      }   //////////////////// PREFECT VP

      // IF ORACLE CONFIDENCE MODE
      if(ORACLE_CONF && vp_eligible){

         // assert SVP enabled(
         assert(SVP_VP); assert(VP->is_SVP_enable()); assert(ORACLE_CONF);
         ...
         // search in SVP
         bool svp_hit = VP->is_SVP_search(PAY.buf[index].pc);
         // also, get confidence (it also checks for svp tag hit)
         //PAY.buf[index].is_Confident = VP->is_Confident(PAY.buf[index].pc);
         PAY.buf[index].pred_DST.dw = VP->prediction(PAY.buf[index].pc);

         // if hit, make a prediction
         if(svp_hit){
            PAY.buf[index].is_Hit = true;
            
            // now check with functional simulator, if it is a good instruction
            db_t* db_entry = get_pipe()->peek(PAY.buf[index].db_index);

            if(PAY.buf[index].good_instruction){
               // good instr and a hit
               if( PAY.buf[index].pred_DST.dw == db_entry->a_rdst[0].value ){
                  PAY.buf[index].is_Confident = true;
               }
               else{
                  PAY.buf[index].is_Confident = false;
      ...
      // IF REAL_VP
      if( SVP_VP && !ORACLE_CONF && !PERF_VP && vp_eligible ){
         //vp_eligible
         PAY.buf[index].vp_eligible = true;

         // allocate this PC to VPQ
         PAY.buf[index].vpq_entry = VP->VPQ_allocate(PAY.buf[index].pc);
         ...
         if(COMPETE){            
            ...
            bool sht_hit = VP->search_SHT( PAY.buf[index].pc );
            ...
            if(svp_hit){
               ...
               if( !PAY.buf[index].is_Confident ){
                  if(sht_hit){
                     PAY.buf[index].is_Confident = true;
                     PAY.buf[index].pred_DST.dw = VP->Competition_prediction( PAY.buf[index].pc ); 
                  }                  
               }
            }
      ...
      // for each instr, check if its a branch instr.
      // if yes, add to it the latest vpq entry and its phase bit
      if( PAY.buf[index].checkpoint && SVP_VP && !ORACLE_CONF && !PERF_VP ){
         //its a branch instr.
         // save vpq tail for recovery
         PAY.buf[index].checkpoint_tail = VP->vpq_tail;
         PAY.buf[index].checkpoint_tp = VP->vpq_tp;
      }
      
   }

   // if (! PAY.buf[index].is_Hit)
   // assert(!PAY.buf[index].is_Confident);
