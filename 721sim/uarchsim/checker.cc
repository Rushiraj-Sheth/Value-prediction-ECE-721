...
void pipeline_t::check_state(state_t *micro_state, state_t *isa_state, db_t *actual) {
   ...
   if (fail) {
    ...
    if (micro_state->badvaddr != isa_state->badvaddr)
        fprintf(stderr, "badvaddr mismatch: micro = 0x%lx, isa = 0x%lx\n", micro_state->badvaddr, isa_state->badvaddr);

    if (micro_state->tohost != isa_state->tohost)
        fprintf(stderr, "tohost mismatch: micro = 0x%lx, isa = 0x%lx\n", micro_state->tohost, isa_state->tohost);

    if (micro_state->fromhost != isa_state->fromhost)
        fprintf(stderr, "fromhost mismatch: micro = 0x%lx, isa = 0x%lx\n", micro_state->fromhost, isa_state->fromhost);

    if (micro_state->count != isa_state->count)
        fprintf(stderr, "count mismatch: micro = %lu, isa = %lu\n", micro_state->count, isa_state->count);

    if (micro_state->sr != isa_state->sr)
        fprintf(stderr, "sr mismatch: micro = 0x%x, isa = 0x%x\n", micro_state->sr, isa_state->sr);

    if (micro_state->fflags != isa_state->fflags)
        fprintf(stderr, "fflags mismatch: micro = 0x%x, isa = 0x%x\n", micro_state->fflags, isa_state->fflags);

    if (micro_state->frm != isa_state->frm)
        fprintf(stderr, "frm mismatch: micro = 0x%x, isa = 0x%x\n", micro_state->frm, isa_state->frm);
...
void pipeline_t::checker() {
   ...
   // If there was an architectural exception, make sure that MICRO_SIM also excepts but don't check anything else.
   if (actual->a_exception) {
      assert(REN->get_exception(PAY.buf[head].AL_index));
      if (PAY.buf[head].split) {
         assert(PAY.buf[head].split_store); // The only split instructions currently supported in 721sim are split-stores.
         assert(PAY.buf[head].upper);       // Only the upper store-addr uop can cause the exception.
         pipe->pop(PAY.buf[head].db_index); // The upper store-addr uop had only peeked, above.  Now we must pop it.
      }
   }
   else {
      if (PAY.buf[head].split) {
         // The only split instructions currently supported in 721sim are split-stores.
         assert(PAY.buf[head].split_store);

         if (PAY.buf[head].upper) {
            // Validate address.
            if (actual->a_addr.valid)
               check_single((reg_t) PAY.buf[head].addr, (reg_t) actual->a_addr.addr, actual, "store-addr uop: address mismatch.");

            // Validate address source register.
            check_single(PAY.buf[head].A_value.dw, actual->a_rsrc[0].value, actual, "store-addr uop: address source register mismatch.");
         }
         else {
            // Validate value source register.
            check_single(PAY.buf[head].A_value.dw, actual->a_rsrc[1].value, actual, "store-val uop: value source register mismatch.");
         }
      }
      else if (IS_MEM_OP(PAY.buf[head].flags)) {
         // Validate address.
         if (actual->a_addr.valid)
            check_single((reg_t) PAY.buf[head].addr, (reg_t) actual->a_addr.addr, actual, "Load/store address mismatch.");
         ...
         if ((IS_STORE(PAY.buf[head].flags))) {
            if (IS_AMO(PAY.buf[head].flags)) {
               check_single(PAY.buf[head].C_value.dw, actual->a_rdst[0].value, actual, "Store conditional destination mismatch.");
               if (actual->a_rsrc[1].valid) {
                  check_single(PAY.buf[head].B_value.dw, actual->a_rsrc[1].value, actual, "Store second source mismatch.");
               }
               else {
                  // In case of a failed store-conditional, the ISA sim never have a chance to capture the RS2
                  assert(actual->a_rdst[0].valid && (actual->a_rdst[0].value != 0)); // SC failed: written non-zero value to rd
               }
            }
            else {
