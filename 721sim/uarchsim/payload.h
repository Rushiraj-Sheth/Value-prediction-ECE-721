...
typedef struct {
   ...
   //bool good_pred_DST = false;    // if "true", we have predicted value for "valid" dest reg. 
   union64_t pred_DST;            // store predicted value for current "valid" dest.
   bool vp_eligible = false;

   uint64_t vpq_entry;
   bool is_Confident;
   bool is_Hit;                    // "true" if prediction is available
   bool is_pred_Correct;           // "true" if 
   uint64_t checkpoint_tail;
   uint64_t checkpoint_tp;
