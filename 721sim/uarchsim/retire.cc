...
void pipeline_t::retire(size_t &instret) {
   ...
   if (head_valid && completed) { // AL head exists and completed
      ...
      if (!exception && !load_viol) {
         ...
         //////// train the SVP
         reg_t AL_PC = REN->AL[REN->al_head].pc;
         assert(AL_PC == PAY.buf[PAY.head].pc);

         if( PAY.buf[PAY.head].vp_eligible && !PERF_VP ){
            VP->train_SVP(AL_PC, PAY.buf[PAY.head].C_value.dw, PAY.buf[PAY.head].is_Confident, PAY.buf[PAY.head].vpq_entry);
         }
        ...
        // if( !val_misp ){
            ...
            if( !PAY.buf[PAY.head].vp_eligible ){
               ...
               // 3. check if vp was a hit
               if( !(PAY.buf[PAY.head].is_Hit) ){
                  vpmeas_miss++;
               }
               else{
                  // 4. vp is available & confident
                  if(PAY.buf[PAY.head].is_Confident){
                     // if correct
                     if(PAY.buf[PAY.head].is_pred_Correct){
                        vpmeas_conf_corr++; // confident and correct
                     }
                     else{
                        vpmeas_conf_incorr++; //confident and incorrect
                     }
                  }
                  else{
                     //6. vp is available and not confident
                     if(PAY.buf[PAY.head].is_pred_Correct){
                        vpmeas_unconf_corr++; //not conf and correct
                     }
                     else{
                        vpmeas_unconf_incorr++; //not conf and incorrect
                     }
                  }
               }
            }
        // }
         ...
         if (!PAY.buf[PAY.head].split || !PAY.buf[PAY.head].upper) {
            ...
            ////////////////////////////////////////////////////
               #ifdef DEBUG_PRINTS  // force rebuild
               instr_cnt++;
               INSTR_NUM = num_insn;
               if( instr_cnt>0 and (instr_cnt%1000 == 0) ){
                  fprintf(stats_log, "-------- DEBUG: retired instruction count = %d\n", instr_cnt);
                  VP->svp_debug_prints(stats_log);
                  VP->vpq_debug_prints(stats_log);
               }
               #endif
            ////////////////////////////////////////////////////
         }
