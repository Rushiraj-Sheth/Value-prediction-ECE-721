...
static void help() {
   ...
   fprintf(stderr, "  --ras=<n>,<rec_model>\tRAS has <n> entries. <rec_model>: 0 (restore TOS pointer only), 1 (restore TOS pointer and TOS content), 2 (restore RAS by walking the full log).\n");
   ...
   fprintf(stderr, "  --cbpALG=<n>       The conditional branch predictor algorithm: 0 for gshare, 1 for tage-sc-l (overrides --mbp=1)\n");
   ...
   fprintf(stderr, "  --mdp=<mdp_model>,<mdp_ctr_max>\t<mdp_model>: 0 (always pred. conflict), 1 (always pred. no conflict), 2 (MDP-sticky), 3 (MDP-ctr), 4 (oracle). <mdp_ctr_max>: max counter value for MDP-ctr.\n");
   fprintf(stderr, "  --splitstores=<0/1>\t0: disable split-stores. 1: enable split-stores.\n");
   ...
   fprintf(stderr, "  -L                 ECE 721 Project 1 Limit Study fixed parameters: 16-wide, universal execution lanes with 1-cycle EX, age-based issue, 64 branch checkpoints, FQ=64.\n");
...
static void config_RAS(const char *config) {
   unsigned int n, rec_model;
   if (sscanf(config, "%u,%u", &n, &rec_model) != 2) {
      fprintf(stderr, "Incorrect usage: --ras=<n>,<rec_model>\tRAS has <n> entries. <rec_model>: 0 (restore TOS pointer only), 1 (restore TOS pointer and TOS content), 2 (restore RAS by walking the full log).\n");
      exit(-1);
   }
   else {
      RAS_SIZE = n;
      switch (rec_model) {
      case 0:
         RAS_RECOVER_MODEL = ras_recover_e::RAS_RECOVER_TOS_POINTER;
         break;
      case 1:
         RAS_RECOVER_MODEL = ras_recover_e::RAS_RECOVER_TOS_POINTER_AND_CONTENT;
         break;
      case 2:
         RAS_RECOVER_MODEL = ras_recover_e::RAS_RECOVER_WALK;
         break;
      default:
         fprintf(stderr, "Incorrect usage: --ras=<n>,<rec_model>\tRAS has <n> entries. <rec_model>: 0 (restore TOS pointer only), 1 (restore TOS pointer and TOS content), 2 (restore RAS by walking the full log).\n");
         exit(-1);
...
static void set_vp_flags(const char* config){
   uint64_t  p_intalu, p_fpalu, p_load;
   if (sscanf(config, "%lu,%lu,%lu", &p_intalu, &p_fpalu, &p_load) != 3) {
      fprintf(stderr, "Incorrect usage of --vp-eligible=<p_intalu>,<p_fpalu>,<p_load>\n");
      fprintf(stderr, "...where p_intlau (predict int ALU), p_fpalu (predict FP ALU), p_load (predict LOAD) are each 0 or 1.\n");
      exit(-1);
   }
   else {
      predINTALU = (p_intalu ? true : false);
      predFPALU = (p_fpalu ? true : false);
...
static void set_svp_flags(const char* config){
   uint64_t VPQ_size, oracle_conf, num_svp_index_bits, num_tag_bits, max_conf;
   if (sscanf(config, "%lu,%lu,%lu,%lu,%lu", &VPQ_size, &oracle_conf, &num_svp_index_bits, &num_tag_bits, &max_conf) != 5) {
      fprintf(stderr, "Incorrect usage of --vp-svp=<VPQ SIZE>,<ORACLE CONF>,<# INDEX BITS>,<# TAG BITS>, <MAX CONFIDENCE>\n");
      exit(-1);
   }
   else{
      // SVP_VP = true
      ...
      NUM_INDEX_BITS = num_svp_index_bits;
...
///////////////////
static void set_competition_flags(const char* config){
   uint64_t len_sht, max_sht_conf;
   if (sscanf(config, "%lu,%lu", &len_sht, &max_sht_conf) != 2) {
      fprintf(stderr, "Incorrect usage of --vpcompete=<len_sht>, max_sht_conf\n");
      exit(-1);
   }
   else{
      COMPETE = true;
      LEN_SHT = len_sht;
      SHT_CONF = max_sht_conf;
   }
...
int main(int argc, char **argv) {
   ...
   parser.option(0, "ras", 1, [&](const char *s) { config_RAS(s); });
   parser.option(0, "mbp", 1, [&](const char *s) { COND_BRANCH_PRED_PER_CYCLE = atoi(s); });
   parser.option(0, "cbpALG", 1, [&](const char *s) { CBP_ALGORITHM = atoi(s); });
   ...
   parser.option('L', 0, 0, [&](const char *s) {
      FETCH_WIDTH = 16; DISPATCH_WIDTH = 16; ISSUE_WIDTH = 16; RETIRE_WIDTH = 16;
      set_lane_matrix("0xffff:0xffff:0xffff:0xffff:0xffff:0xffff:0xffff"); set_lane_latencies("1:1:1:1:1:1:1");
   ...
   /////////////////////////////////////////
   parser.option(0, "vp-eligible", 1,[&](const char *s){set_vp_flags(s);} );
   parser.option(0, "vp-perf", 1, [&](const char *s){PERF_VP = true;} );

   // svp
   parser.option(0, "vp-svp", 1, [&](const char *s){set_svp_flags(s);} );

   // competition args
   parser.option(0, "vp-compete", 1, [&](const char *s){set_competition_flags(s);} );
   ////////////////////////////////////////
