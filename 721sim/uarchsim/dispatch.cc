...
void pipeline_t::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      if (IS_LOAD(PAY.buf[index].flags)) {
         ...
         // Split-stores are comprised of a store-addr uop and a store-val uop.
         // The two uops share a SQ entry to "rejoin". Therefore, only the first uop should check
         // for and allocate a SQ entry; the second uop should inherit the same entry.
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      //       as shown below (if split stores are enabled, the other term ensures the store flag is only set for the
      //       second/lower uop of a split store). Use the local variable 'store_flag' (already declared):
      ...
      PAY.buf[index].AL_index = REN->dispatch_inst( PAY.buf[index].C_valid, 
                                                      ...
                                                      load_flag, store_flag, branch_flag,
      ...
      if(false == PAY.buf[index].A_valid){
         A_ready = true;
      }
      ...
      if(false == PAY.buf[index].B_valid){
         B_ready = true;
      }
      ...
      if(false == PAY.buf[index].D_valid){
         D_ready = true;
      }
      // FIX_ME #8 END
      ...
      if(PAY.buf[index].C_valid){
         REN->clear_ready(PAY.buf[index].C_phys_reg); // ; clear the ready bit for all type of dest reg.
      }
      // FIX_ME #9 END

      //////////////////////////////////////////////////////////////////////////////////
      
      //////////////// PERFECT Value prediction injection
      if( PAY.buf[index].vp_eligible && PAY.buf[index].is_Confident && PERF_VP ){
         // set PRF ready bit for value predicted instruction
         REN->set_ready( PAY.buf[index].C_phys_reg );

         // write the predicted value in PRF
         REN->write( PAY.buf[index].C_phys_reg, PAY.buf[index].pred_DST.dw );
      } ///////// PERF VP


      //////////////// ORACLE CONFIDENCE MODE
      if( ORACLE_CONF && PAY.buf[index].vp_eligible && PAY.buf[index].is_Confident ){
         assert(!PERF_VP); // not perfect VP
         
         // inject the predicted value in PRF
         REN->set_ready(PAY.buf[index].C_phys_reg);
         REN->write( PAY.buf[index].C_phys_reg, PAY.buf[index].pred_DST.dw);
      } //////// ORACLE CONFIDENCE MODE


      //////////////// REAL CONFIDENCE MODE
      if( SVP_VP && !ORACLE_CONF && !PERF_VP && PAY.buf[index].is_Confident ){
         assert( PAY.buf[index].vp_eligible );

         // confident --> inject the VP
         REN->set_ready( PAY.buf[index].C_phys_reg );
         REN->write( PAY.buf[index].C_phys_reg, PAY.buf[index].pred_DST.dw );
      } /////////////// REAL CONFIDENCE MODE
         ...
         if (!PAY.buf[index].split_store || PAY.buf[index].upper) {
            ...
            if (ORACLE_DISAMBIG && PAY.buf[index].good_instruction && IS_STORE(PAY.buf[index].flags)) {
               ...
               if (actual->a_addr.valid) {
                  LSU.store_addr(cycle, actual->a_addr.addr, PAY.buf[index].SQ_index, PAY.buf[index].LQ_index, PAY.buf[index].LQ_phase);
               }
               else {
                  // In some cases the a_addr might not be available, because ISA sim didn't have a chance to push the address to the debug buffer.
                  // Some scenario for example:
                  // 1. A failed store-conditional instruction
                  // 2. An FP store instruction that triggered FP_DISABLED exception
                  assert(IS_AMO(PAY.buf[index].flags) || IS_FP_OP(PAY.buf[index].flags));
               }
            }
         }
      }
