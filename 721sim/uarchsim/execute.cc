...
void pipeline_t::execute(unsigned int lane_number) {
      ...
      if (IS_MEM_OP(PAY.buf[index].flags)) {
         ...
         if (IS_LOAD(PAY.buf[index].flags)) {
            ...
            if( (hit) && (PAY.buf[index].C_valid) ){
               ...
               if( !PAY.buf[index].vp_eligible ){
                  ...
                  // see for vp eligible instr
                  // if confident
                  if(PAY.buf[index].is_Confident){
                     assert(PAY.buf[index].vp_eligible);
                     //check predictrion
                     if(PAY.buf[index].C_value.dw != PAY.buf[index].pred_DST.dw){
                        ...
                        //post val_misp
                        REN->AL[PAY.buf[index].AL_index].val_mispred_bit = true;
                     }
                     else{
                        PAY.buf[index].is_pred_Correct = true;
                     }
                  }
                  else{
                     // we are not confident
                     // so didn't inject prediction in dispatch
                     // write computed val to PRF
            ...
            // Store-conditional: will be executed when being committed at the retire stage and update its destination register based on the execution result.
         }
      }
      else {
         ...
         if( PAY.buf[index].C_valid ){
            ...
            if(!PAY.buf[index].vp_eligible){
               ...
               // vp_eligible
               // if confident but wrong prediction
               if(PAY.buf[index].is_Confident){
                  assert(PAY.buf[index].vp_eligible);
                  //check predictrion
                  if(PAY.buf[index].C_value.dw != PAY.buf[index].pred_DST.dw){
                  ...
                  // vp_eligible and not confident case
                  if(PAY.buf[index].C_value.dw == PAY.buf[index].pred_DST.dw){
                     PAY.buf[index].is_pred_Correct = true;
                  }
                  else{
   ...
   if (depth > 0) {
      ...
      if (Execution_Lanes[lane_number].ex[depth - 1].valid) {
         ...
         if( (PAY.buf[index].C_valid) && (!load_flag) && (!amo_flag) ){

            if( !(PAY.buf[index].vp_eligible && PAY.buf[index].is_Confident) ){
               IQ.wakeup(PAY.buf[index].C_phys_reg);
            }

            REN->set_ready(PAY.buf[index].C_phys_reg); // We are only sending to signals to 
            // make ready the dependent sources in IQ for dest. being calculated in current cycle
         }
         // FIX_ME #11b END
      }
   }
      ...
      if (PAY.buf[index].C_valid) {
         ...
         if(!PAY.buf[index].vp_eligible){
            ...
            if(PAY.buf[index].is_Confident){
               ...
               // we predicted but are not confident
               // so didn't inject prediction in dispatch
               // write computed val to PRF
         ...
         // for VP eligible instruction, add the computed value to VPQ
         if( !PERF_VP && PAY.buf[index].vp_eligible ){
            //assert that the VPQ entry PC must MATCH AL.pc
            int AL_ID = PAY.buf[index].AL_index;
            int AL_PC = REN->AL[AL_ID].pc;
            assert( AL_PC == VP->VPQ[ PAY.buf[index].vpq_entry ].PC );

            // insert the value to instr's vpq_tail
            VP->VPQ[ PAY.buf[index].vpq_entry ].value = PAY.buf[index].C_value.dw; 
