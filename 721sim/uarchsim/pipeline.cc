...
#undef OPEN_LOG_FILE
   ...
   if (AUTO_BQ_SIZE) { // If true, the user didn't pass in a bq size argument. Thus, auto-size the BQ based on other parameters.
   ...
   // Override the number of conditional branch predictions per cycle if using TAGE-SC-L.
   if (CBP_ALGORITHM == 1) {
      COND_BRANCH_PRED_PER_CYCLE = 1;
   }
   ...
   ///////////////////////////////////////////////////////////////
   // Set up for Value Predicton Object Creation... You can add on params here as you like.
   ///////////////////////////////////////////////////////////////
   if(!COMPETE){
      VP = new Value_Prediction(SVP_VP, ORACLE_CONF, NUM_INDEX_BITS, NUM_TAG_BITS, VPQ_SIZE, MAX_CONF); //PRED_INT_ALU, PRED_FP_ALU, PRED_LOAD);
   }
   else{
      VP = new Value_Prediction(SVP_VP, ORACLE_CONF, NUM_INDEX_BITS, NUM_TAG_BITS, VPQ_SIZE, MAX_CONF,LEN_SHT, SHT_CONF);
   }
   /////////////////////////////////////////////////////////////
   ...
   fprintf(stats_log, "   SPLIT STORES = %d\n", (SPLIT_STORES ? 1 : 0));
   ...
   if (L2_PRESENT) {
      ...
      if (!L3_PRESENT) fprintf(stats_log, "   miss latency = %d cycles\n", L2_MISS_LATENCY);
   ...
   switch (RAS_RECOVER_MODEL) {
   case ras_recover_e::RAS_RECOVER_TOS_POINTER:
      fprintf(stats_log, "RAS_RECOVER_MODEL = RAS_RECOVER_TOS_POINTER\n");
      break;
   case ras_recover_e::RAS_RECOVER_TOS_POINTER_AND_CONTENT:
      fprintf(stats_log, "RAS_RECOVER_MODEL = RAS_RECOVER_TOS_POINTER_AND_CONTENT\n");
      break;
   case ras_recover_e::RAS_RECOVER_WALK:
      fprintf(stats_log, "RAS_RECOVER_MODEL = RAS_RECOVER_WALK\n");
      break;
   default:
      assert(0);
      break;
   ...
   fprintf(stats_log, "CBP_ALGORITHM = %s\n", ((CBP_ALGORITHM == 0) ? "gshare" : "tage-sc-l"));
   ...
   /////////////////////////////////////////////
   //Function Call for printing VP cost
   VP->VP_Debug_Prints(stats_log);
   /////////////////////////////////////////////
   ...
   ///////////////////////////////////////////////////
   // VPU MEASUREMENTS
   //VP->VP_stats(stats_log);
   ///////////////////////////////////////////////////

   
   ///////////////////////////////////////////////////
   // SVP AND VPQ DEBUG PRINTS
   //
   ///////////////////////////////////////////////////
...
bool pipeline_t::eligible(payload_t *pay){
   ...
   // any instruction w/o a destination reg. is ineligible
   if(!pay->C_valid){
      return false;
   }

   // if we reached at this point, the instructuion has a valid destinatin reg.
